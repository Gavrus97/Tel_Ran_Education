...
class CustomArrayListTest extends CustomListTest {
    ...
    public void testSort_integers(){
        ...
        CustomArrayList<Integer> intArrayList = (CustomArrayList<Integer>) intList; //даункастинг
        intArrayList.sort(new NaturalIntComparator());

        int[] expected = {5, 6, 10, 15, 17};

        assertListContents(expected); //в классе CustomListTest метод сделали protected и здесь получили доступ к нему
